...
% watermark2 为由水印模板直接重构得到的水印形态, 仅便于直观认识,
%本身无 意义。
% correlationU, correlationV 为替换正交矩阵后与未替换的正交矩阵的相关系数
function [ watermarkimagergb, watermarkimage, waterCA, watermark2, correlationU, correlationV] = water( input, goal, seed, wavelet, level, alpha, ratio)
...
d = max( size( CA) ) ; % 对低频率系数单值分解
[ U, sigma, V] = svd( CA) ; % 按输出参数得到要替换的系数的数量
np = round( d* ratio) ; % 以下是随机正交矩阵的生成
rand( 'seed', seed) ;
M_V = rand( d, np) - 0.5;
[ Q_V, R_V] = qr( M_V, 0) ;
M_U = rand( d, np) - 0.5;
...
waterC = C; waterC( 1, 1: S( 1, 1) * S( 1, 2) ) = watermark2;
watermark2 = waverec2 ( waterC, S, wavelet) ;
% 调整系数生成嵌入水印后的图像
CA_tilda = CA + watermark;
over1 = find( CA_tilda > 1) ;
below0 = find( CA_tilda < 0) ;
CA_tilda( over1) = 1; CA_tilda( below0 ) = 0;% 系数调整, 将过幅系数与负数修正
CA_tilda = ( CAmax-CAmin) * CA_tilda +CAmin;
% 系数还原到归一化以前的范围
% 记录加有水印的低频系数
waterCA = CA_tilda;
if row <= list waterCA = waterCA( 1:Sreal( 1, 1) ,:) ;
else waterCA = waterCA(:, 1: Sreal( 1, 2) ) ;
end
% 重构
...
else watermarkimage = watermarkimage(:, 1:list) ;
end
...
% figure( 1) ;
% axes(axesWSVD);%展示隐写结果
% subplot( 221) ; imshow( watermark2* 255) ; title( 'water') ; 
% subplot( 223) ; imshow( data) ; title( 'origin') ; 
% subplot( 224) ; imshow( watermarkimagergb2 ) ; title( 'water in RGB') ; 
% subplot( 325) ; imshow( datared) ; title( 'Red-layout') ;
% subplot( 326) ; imshow( watermarkimage) ; title( 'water-R') ;