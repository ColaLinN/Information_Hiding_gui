...
%count为待隐藏信息的长度
%result为隐藏结果
function [count,msg,result]=hidedctadv1(image,imagegoal,msg,key,alpha,string)
fid=fopen('read.txt','w+');%打开read文件把文本框信息重写read.txt
fprintf(fid,'%s',string);
fclose(fid);
frr=fopen(msg,'r');
[msg,count]=fread(frr,'ubit1');%按二进制位读取秘密信息
fclose(frr);
data0=imread(image);
%将图象矩阵转为double型
